...
| A-Q9 | **`IInventoryService` cannot delete a folder that is not in Trash, and does not say so.** The only overload on the interface is `DeleteFolders(userID, folderIDs)`, which is the `onlyIfTrash = true` form: it `continue`s past any folder whose parent is not Trash or Lost And Found and still returns `true` (`XInventoryService.cs:459-478`). The LL viewer calls `RemoveCategory` on **any** non-protected folder, wherever it sits (`llviewerinventory.cpp:1545-1568`), so `DELETE /category/{id}` cannot be honoured for most folders. | **John** | A5 (advertising the cap): with AIS on, a folder delete has no fallback (§1g, A-R1) | **Open.** A2 does not paper over it: the handler re-reads the folder after the delete and answers **409 Conflict** with a message naming the reason instead of a false 200. Three ways out, none of them A2's to pick: (a) add the three-argument overload to `IInventoryService` and pass `onlyIfTrash: false`; (b) have the handler move the folder to Trash and then delete it, which is two writes and changes what the user sees if the second fails; (c) leave it, and accept that LL-viewer folder deletion does not work on this grid. |
